...
def ensure_schema(con: duckdb.DuckDBPyConnection) -> None:
    """Create episode tables if they don't already exist."""
    con.execute(_SCHEMA_SQL)


def _get_episode_connection(
    db_path: str | Path = DEFAULT_DB_PATH,
) -> duckdb.DuckDBPyConnection:
    """Open a connection and ensure the episode schema exists."""
...
def save_episode(episode: Episode, db_path: str | Path = DEFAULT_DB_PATH) -> None:
    ...
    con = _get_episode_connection(db_path)
    try:
        con.begin()
        gid = episode.game_id
...
def saved_game_ids(db_path: str | Path = DEFAULT_DB_PATH) -> set[int]:
    """Return the set of game IDs already stored in the database."""
    con = _get_episode_connection(db_path)
    try:
        rows = con.execute("SELECT game_id FROM episodes").fetchall()
