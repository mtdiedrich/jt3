...
def save_embeddings(
    ...
    *,
    ...
    """Bulk-save text→embedding pairs into ``embeddings.<table>``."""
    tbl = _validate_identifier(table)
    dim = embeddings.shape[1]
    con = get_connection(db_path)
    try:
        con.execute("CREATE SCHEMA IF NOT EXISTS embeddings")
        con.execute(f"DROP TABLE IF EXISTS embeddings.{tbl}")
        con.execute(
            f"CREATE TABLE embeddings.{tbl} "
            f"(text TEXT PRIMARY KEY, embedding FLOAT[{dim}] NOT NULL)"
        )
        df = pl.DataFrame(
            {"text": texts, "embedding": embeddings.tolist()}
        ).unique(subset=["text"], keep="first")
        con.execute(
            f"INSERT INTO embeddings.{tbl} SELECT text, embedding FROM df"
        )
    finally:
        con.close()
...
def get_embedding(
    ...
    *,
    ...
    try:
        row = con.execute(
            f"SELECT embedding FROM embeddings.{tbl} WHERE text = ?", [text]
        ).fetchone()
        if row is None:
            return None
        return list(row[0])
